...
```
OTP Buddy is a macOS menu-bar (agent) app. There is no Dock icon by default (LSUIElement). Opening the main window temporarily shows a Dock icon (menu-bar pin otherwise).

NO LOGIN / DEMO ACCOUNT REQUIRED beyond the user’s own IMAP credentials (stored in Keychain).
...
It only registers as a Login Item if the user chooses Open at Login. Later launches stay menu-bar only (Guideline 2.4.5(iii)).

How to review:
1. Launch the app. Look for the OTP icon in the macOS menu bar.
2. Open Settings and enter IMAP host/credentials (or use Demo: ingest sample OTP email).
3. Confirm codes appear in the menu bar; optional auto-copy is off by default.

Privacy:
- App connects to user-provided IMAP host with user credentials (Keychain).
- Reads recent inbox messages locally to detect verification codes.
- Does not send email contents to our servers.
```
...
**Name:** OTP Buddy  
**Subtitle:** Email codes to your clipboard  
**Keywords:** OTP,2FA,email,verification,IMAP,clipboard,menu bar  
**Promotional Text:** OTP codes from your inbox to the menu bar — optional one-click copy, credentials in Keychain.

**Description:**

OTP Buddy watches a mailbox you connect (IMAP) and surfaces one-time passcodes in the menu bar the moment they arrive.

Optionally auto-copy codes to the clipboard, or confirm with one click. Credentials live in the Keychain; codes are held briefly in memory and never uploaded.
...
OTP Buddy überwacht ein von Ihnen verbundenes Postfach (IMAP) und zeigt Einmal-Codes in der Menüleiste, sobald sie eintreffen.

Optional kopieren Sie Codes automatisch in die Zwischenablage oder bestätigen mit einem Klick. Zugangsdaten liegen im Schlüsselbund; Codes bleiben kurz im Speicher und werden nie hochgeladen.
